        	...
        	SELECT	E.OPRAT_CODE		/*운행코드*/,
  			...
  			FROM
       			(
                	SELECT	A.OPRAT_CODE,
                    		C.TRAIN_NO,
                    		C.TRAIN_KND AS TRAIN_KND_CODE,
              				PK_CMMN.TO_CODE_VALUE('CMMN_CODE', C.TRAIN_KND) AS TRAIN_KND_VALUE,
            		...
            		AND		C.TRAIN_CODE = A.TRAIN_CODE
                    AND		c.TRAIN_KND = I_TRAIN_KND_CODE
                    AND 	A.START_STATN = I_START_STATN_CODE
                    AND 	A.ARVL_STATN = I_ARVL_STATN_CODE
                    AND 	TO_CHAR(A.START_TM, 'YYYY-MM-DD HH24') >= I_START_TM
       			) E
