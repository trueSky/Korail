CREATE OR REPLACE PACKAGE BODY KORAIL.PK_ADMIN_OPRAT AS
    ...
    **********************************************************/
	...
	BEGIN
    	...
    	ELSIF I_TRAIN_NO IS NOT NULL THEN
    ...
    * 이름        : SP_DLT_OPRAT
    * 설명        : 운행일정 삭제
    * 관련테이블  : OPRAT, DETAIL_OPRAT, ROOM, RESVE
    **********************************************************/
    PROCEDURE SP_DLT_OPRAT(
    	I_OPRAT_CODE IN VARCHAR2, /*운행일정코드*/

        ER_CODE OUT VARCHAR2	/*에러 코드(0 : 성공, 1 : 에러)*/,
        ER_MSG 	OUT VARCHAR2	/*에러 메세지*/
    )
    IS
    	V_COUNT NUMBER(10);		/*카운트*/
    BEGIN
    	/*예약된 승차권 확인*/
    	SELECT COUNT(OPRAT_CODE)
        INTO V_COUNT
        FROM RESVE
        WHERE OPRAT_CODE =  I_OPRAT_CODE
        ;

        /*예약된 승차권이 없을 때 운행일정 삭제*/
        IF V_COUNT = 0 THEN
        	/*호실 삭제*/
            DELETE FROM ROOM WHERE OPRAT_CODE = I_OPRAT_CODE;
            IF SQL%FOUND THEN
            	/*상세운행일정 조회*/
                SELECT COUNT(OPRAT_CODE)
        		INTO V_COUNT
        		...
        		WHERE OPRAT_CODE =  I_OPRAT_CODE
       			;
            	/*상세운행일정 삭제*/
            	DELETE FROM DETAIL_OPRAT WHERE OPRAT_CODE = I_OPRAT_CODE;
                IF SQL%FOUND OR V_COUNT = 0 THEN
                	/*운행일정 삭제*/
                    DELETE FROM OPRAT WHERE OPRAT_CODE = I_OPRAT_CODE;
